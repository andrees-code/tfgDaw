<template>
  <div>
    ...
    <div class="max-w-6xl mx-auto p-4 flex flex-col md:flex-row gap-6">
      ...
      <!-- 🗂 MENÚ ASIGNATURAS -->
      <aside
        class="w-full md:w-64 border rounded p-4 flex flex-col"
      >
        ...
        <div v-if="asignaturas.length">
          ...
          >
            ...
            >
              ...
              <li
                v-if="!examsByAsignatura(asig).length"
        ...
        <!-- Botón cuando no hay asignaturas -->
        <div
          v-else
      ...
      <main class="flex-1">
        ...
        <div
          ...
          :key="exam._id"
          class="border rounded-lg p-4 mb-4 shadow-sm hover:shadow-md transition
        ...
        >
          ...
          <div class="flex-1">
            <input
              v-model="exam.title"
              @blur="saveTitle(exam)"
              class="text-lg font-semibold w-full border-b border-gray-300
                     focus:outline-none focus:border-blue-500 mb-2"
            />
...
const filteredExams = computed(() => {
  return tab.value === 'favoritos'
    ? exams.value.filter(e => e.favorito)
    : exams.value
})
