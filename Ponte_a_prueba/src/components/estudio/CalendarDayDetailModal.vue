    ...
    <div class="relative w-full max-w-lg max-h-[85vh] flex flex-col bg-slate-900/95 backdrop-blur-2xl rounded-3xl border border-white/10 shadow-2xl ring-1 ring-white/5 overflow-hidden animate-pop-in">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-white/10 shrink-0">
        ...
        <div class="flex items-center gap-1 shrink-0">
          <button type="button" class="flex items-center gap-1 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition-colors" aria-label="Añadir o editar en el día" @click="openAddMenu">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar px-6 py-5 space-y-6">
        ...
        <section v-if="events.length">
          ...
          <div class="space-y-1.5">
            <div v-for="ev in events" :key="ev.id" class="px-3 py-2 rounded-xl bg-slate-950/50 border border-white/5 group">
              <!-- Modo edición -->
              <div v-if="editingId === ev.id" class="flex flex-col gap-2">
                <div class="flex gap-2">
                  <input v-model="editText" type="text" class="flex-1 min-w-0 bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-indigo-500/50" aria-label="Título del evento" />
                  <input v-model="editTime" type="time" class="bg-slate-900 border border-white/10 rounded-xl px-2 py-2 text-sm text-slate-300 focus:outline-none focus:border-indigo-500/50" aria-label="Hora del evento" />
                </div>
                <div class="flex items-center justify-between gap-2">
                  ...
                  <div class="flex gap-1.5">
                    <button type="button" class="px-3 py-1.5 text-xs font-bold text-slate-400 hover:text-slate-200 rounded-xl transition-colors" @click="cancelEdit">Cancelar</button>
                    <button type="button" :disabled="!editText.trim()" class="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-xs font-bold rounded-xl transition-colors" @click="saveEdit(ev)">Guardar</button>
                  </div>
                </div>
              </div>
              <!-- Modo lectura -->
        ...
        <section v-if="texts.length">
          ...
          <div class="space-y-1.5">
            <div v-for="tx in texts" :key="tx.id" class="px-3 py-2 rounded-xl border group" :style="{ borderColor: (tx.color || '#94a3b8') + '55', backgroundColor: (tx.color || '#94a3b8') + '15' }">
              <div v-if="editingId === tx.id" class="flex flex-col gap-2">
                <input v-model="editText" type="text" class="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-indigo-500/50" aria-label="Texto de la nota rápida" />
                <div class="flex items-center justify-between gap-2">
                  <div class="flex gap-1.5">
                    <button v-for="c in colorPresets" :key="c" type="button" class="w-5 h-5 rounded-full transition-transform hover:scale-110" :style="{ backgroundColor: c }" :class="editColor === c ? 'ring-2 ring-white ring-offset-2 ring-offset-slate-950' : ''" :aria-label="'Color ' + c" @click="editColor = c"></button>
                  </div>
                  <div class="flex gap-1.5">
                    <button type="button" class="px-3 py-1.5 text-xs font-bold text-slate-400 hover:text-slate-200 rounded-xl transition-colors" @click="cancelEdit">Cancelar</button>
                    <button type="button" :disabled="!editText.trim()" class="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-xs font-bold rounded-xl transition-colors" @click="saveEdit(tx)">Guardar</button>
                  </div>
                </div>
              </div>
              <div v-else class="flex items-center gap-2">
        ...
        <!-- DOCUMENTOS Y POST-ITS -->
        <section v-if="linkedNotes.length">
          <h3 class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-2">Documentos y post-its</h3>
          <div class="space-y-1.5">
            <div v-for="note in linkedNotes" :key="note.id" class="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-950/50 border border-white/5 group">
              ...
              <button type="button" class="p-1 text-slate-600 hover:text-indigo-300 opacity-0 group-hover:opacity-100 transition-all" title="Abrir" aria-label="Abrir" @click="openNote(note)">
...
import {
  ...
  ExternalLink as IconOpen, FileText as IconFile, StickyNote as IconSticky,
